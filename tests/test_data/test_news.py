...
async def test_afetch_company_news(sample_news_response):
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_response = Mock()
        ...
        fetcher = NewsFetcher(api_key="test-key")
        articles = await fetcher.afetch_company_news("AAPL", limit=10)
...
@pytest.mark.asyncio
async def test_afetch_company_news_no_api_key(monkeypatch):
    monkeypatch.delenv("MARKETAUX_API_KEY", raising=False)
    ...
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        ...
        await fetcher.afetch_company_news("AAPL")
...
@pytest.mark.asyncio
async def test_afetch_company_news_http_error():
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_client = AsyncMock()
...
@pytest.mark.asyncio
async def test_afetch_market_news_http_error():
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_client = AsyncMock()
        mock_client.get = AsyncMock(side_effect=httpx.HTTPError("API Error"))
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        ...
        with pytest.raises(httpx.HTTPError):
            await fetcher.afetch_market_news()
...
@pytest.mark.asyncio
async def test_afetch_company_news_retries_on_timeout(sample_news_response):
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_response = Mock()
        mock_response.json.return_value = sample_news_response
        mock_response.raise_for_status = Mock()
        ...
        fetcher = NewsFetcher(api_key="test-key")
        articles = await fetcher.afetch_company_news("AAPL")
...
@pytest.mark.asyncio
async def test_afetch_company_news_retries_on_connection_error(sample_news_response):
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_response = Mock()
        mock_response.json.return_value = sample_news_response
        mock_response.raise_for_status = Mock()
...
@pytest.mark.asyncio
async def test_afetch_company_news_exhausts_retries():
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_client = AsyncMock()
...
@pytest.mark.asyncio
async def test_afetch_market_news_retries_on_timeout(sample_news_response):
    with patch("src.data.news.httpx.AsyncClient") as mock_client_class:
        mock_response = Mock()
        mock_response.json.return_value = sample_news_response
        mock_response.raise_for_status = Mock()
        ...
        fetcher = NewsFetcher(api_key="test-key")
        articles = await fetcher.afetch_market_news()
