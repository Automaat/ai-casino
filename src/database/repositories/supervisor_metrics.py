...
class SupervisorMetricsRepository(BaseRepository[SupervisorCycleMetrics]):
    ...
    def _to_metrics(self, orm: SupervisorMetricsORM) -> SupervisorCycleMetrics:
        ...
        return SupervisorCycleMetrics(
            ...
            skip_analyses={str(k): str(v) for k, v in dict(orm.skip_analyses).items()},
            ...
            worker_timings={str(k): float(v) for k, v in dict(orm.worker_timings).items()},
            worker_errors={str(k): str(v) for k, v in dict(orm.worker_errors).items()},
